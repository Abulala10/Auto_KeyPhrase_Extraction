...
"""
from ANN.keywords import extract_candidate_keywords
...
def get_features(document, candidate, candidate_list, isKeyword):
    features = {'length': len(candidate.split(' ')),
                'part_of_speech': ' '.join([pos for word, pos in nltk.pos_tag(nltk.word_tokenize(candidate))])}
    # features['part_of_speech'] prints NN IN NNP NNPS that is noun (NN) or preposition (IN)
    position_list = [m.start() / float(len(document)) for m in
                     re.finditer(re.escape(candidate), document, flags=re.IGNORECASE)]
...
for doc in os.listdir(doc_path):
    ...
    if doc.endswith('txt'):
        ...
        document = bytes(document + doc, errors='replace', encoding="utf8").decode()
        # document = "".encode(document + doc,errors='replace')
        ...
        for team in os.listdir(team_path):
            ...
            if team.startswith('team'):
                ...
                # keywords = "".encode(keywords + team + '/' + doc_name + '.key',errors='replace')
                keywords = bytes(keywords + team + '/' + doc_name + '.key', errors='replace', encoding="utf8").decode()
        ...
        feature_list.extend(
            [(get_features(document, key, candidates, 0), 0) for key in candidates if key not in set(keyword_list)])
